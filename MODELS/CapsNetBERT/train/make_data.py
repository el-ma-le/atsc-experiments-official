...
def make_term_data(config):
    ...
    if rand_state == 0:
        ...
        train_path = os.path.join(base_path, 'processed/train_'+str(rand_state)+'.npz')
        val_path = os.path.join(base_path, 'processed/val_'+str(rand_state)+'.npz')
...
def make_term_test_data_arts(config):
    base_path = config['base_path']
    test_path = os.path.join(base_path, 'processed/test.npz')
    test_input_list = input_list[config['aspect_term_model']['type']].copy()
    test_input_list += ['ids']
    test_data = ABSADataset(test_path, test_input_list)
    config = config['aspect_term_model'][config['aspect_term_model']['type']]
    test_loader = DataLoader(
        dataset=test_data,
        batch_size=config['batch_size'],
