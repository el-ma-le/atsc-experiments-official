...
def init_args():
    ...
    parser.add_argument("--train_seed", type=int, default=0, help="choose one of train/val sets for semeval data")
    parser.add_argument("--patience", type=int, default=1, help="number of epochs to wait before early stopping after best val results")
...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    mlflow.log_metric('train_loss', (tr_loss - logging_loss)/args.logging_steps, global_step)
        ...
        val_results = evaluate(args, model, tokenizer, mode='dev', prefix=global_step)
        for key,val in val_results.items():
            mlflow.log_metric("val_"+key, val, epoch) 

        if max_val_f1_macro <= val_results['macro-f1']:
            max_val_f1_macro = val_results['macro-f1']
            max_val_epoch = epoch
            best_model = copy.deepcopy(model)
            best_model_dict = copy.deepcopy(model.state_dict()) 

        if max_val_f1_micro <= val_results['micro-f1']:
            max_val_f1_micro = val_results['micro-f1']
            max_val_epoch = epoch
            best_model = copy.deepcopy(model)
            best_model_dict = copy.deepcopy(model.state_dict()) 

        # if a maximum number of steps is given instead of epochs
        if args.max_steps > 0 and global_step > args.max_steps:
    ...
    return global_step, tr_loss / global_step, best_model_dict
...
def evaluate(args, model, tokenizer, mode, prefix=""):
    ...
    # prepare data
    eval_dataset, eval_evaluate_label_ids = load_and_cache_examples(args, eval_task, tokenizer, args.train_seed, mode=mode)
    ...
    out_label_ids = None
    crf_logits, crf_mask = [], []
    for batch in tqdm(eval_dataloader, desc="Evaluating "+mode):
        model.eval()
        batch = tuple(t.to(args.device) for t in batch)
...
def load_and_cache_examples(args, task, tokenizer, train_seed, mode='train'):
    processor = processors[task]()
    # Load data features from cache or dataset file
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}_split{}'.format(
        mode,
    ...
    if os.path.exists(cached_features_file):
        ...
        # get list of all possible tags according to tagging schema
        label_list = processor.get_labels(args.tagging_schema)
        # for each subset, split each data line into sentence and tags, convert labels into correct schema
        # examples = [identifier-string, text string, None, label list]
        if mode == 'train':
            examples = processor.get_train_examples(args.data_dir, args.tagging_schema, train_seed)
        elif mode == 'dev':
            examples = processor.get_dev_examples(args.data_dir, args.tagging_schema, train_seed)
        elif mode == 'test':
            examples = processor.get_test_examples(args.data_dir, args.tagging_schema)
        else:
            raise Exception("Invalid data mode %s..." % mode)
        
        # tokenize words, create token-wise labels, add SEP and CLS tokens, convert tokens and labels to ids, pad
    ...
    # per sentence: [1,2, ... , number of words], 
    # indices are missing if a word can be tokenized into subwords
    # starts with 1 as [CLS] does not count as a word
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset, train_evaluate_label_ids = load_and_cache_examples(args, args.task_name, tokenizer, args.train_seed, mode='train')
        global_step, tr_loss, best_model_dict = train(args, train_dataset, model, tokenizer)

    # Testing
    model.load_state_dict(best_model_dict)
    tokenizer = tokenizer_class.from_pretrained("./tokenizer")
    test_results = evaluate(args, model, tokenizer, mode='test', prefix=global_step)
    ...
    print('>> test_f1_micro: {:.4f}, test_f1_macro: {:.4f}'.format(test_results['micro-f1'], test_results['macro-f1']))
