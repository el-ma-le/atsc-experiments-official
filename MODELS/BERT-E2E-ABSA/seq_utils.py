...
def tag2ts(ts_tag_sequence):
    ...
    :return:
    """
    # number of tags in the given sentence
    n_tags = len(ts_tag_sequence)
    # list of triples (beg,end,sentiment) to be returned
    ...
    for i in range(n_tags):
        ...
        ts_tag = ts_tag_sequence[i]
        # split tag into position and sentiment
        ...
        # add non-O sentiment to sentiment list
        if sentiment != 'O':
            # current word is a subjective word
            sentiments.append(sentiment)

        # depending on the positional tag (B,I,E,S)
        # decide what to do with sentiments:
        # for single tokens, the sentiment list is emptied and
        # the returning triple is twice the index of the tag
        if pos == 'S':
            ts_sequence.append((i, i, sentiment))
            sentiments = []
        # if the tag is a beginning tag, only keep the last sentiment in the list
        # which is the one of the current tag
        elif pos == 'B':
            beg = i
            if len(sentiments) > 1:
                # remove the effect of the noisy I-{POS,NEG,NEU}
                sentiments = [sentiments[-1]]
        # if the tag is a end-tag and all collected sentiments are the same,
        # return (beg,end,sentiment)
        elif pos == 'E':
            end = i
