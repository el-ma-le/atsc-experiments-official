...
class SeqInputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, evaluate_label_ids, sentence_ids=None):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
...
class ABSAProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, tagging_schema, train_seed):
        return self._create_examples(data_dir=data_dir, set_type='train', tagging_schema=tagging_schema, train_seed=train_seed)

    def get_dev_examples(self, data_dir, tagging_schema, train_seed):
        return self._create_examples(data_dir=data_dir, set_type='val', tagging_schema=tagging_schema, train_seed=train_seed)
    ...
    def _create_examples(self, data_dir, set_type, tagging_schema, train_seed=None):
        examples = []
        if "ARTS-LAP" in data_dir or "ARTS-REST" in data_dir:
            arts_data = True
        else:
            arts_data = False

        if "MAMS" in data_dir or "ARTS-LAP" in data_dir or "ARTS-REST" in data_dir:
            file = os.path.join(data_dir, "%s.txt" % set_type)
        else:
            if set_type == "test":
                file = os.path.join(data_dir, "%s.txt" % set_type)
            else:
                file = os.path.join(data_dir, "%s_%s.txt" % (set_type, train_seed))
        ...
        with open(file, 'r', encoding='UTF-8') as fp:
            ...
            for line in fp:
                if arts_data == True:
                    sent_string, tag_string, id_string = line.strip().split('####')
                else:
                ...
                if arts_data == True:
                    id = float(id_string)
                    examples.append(InputExample(guid=guid, text_a=text_a, text_b=id, label=tags))
                else:
...
def convert_examples_to_seq_features(examples, label_list, tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, label in zip(words, example.label):
            ...
            # create labels for subwords
            # => labels_a has as many elements as there are subwords in a sentence
            # for tokenizable aspect, the first token is assigned the real label,
            # the following tokens just receive "EQ"
            if label != 'O':
                labels_a.extend([label] + ['EQ'] * (len(subwords) - 1))
            else:
                labels_a.extend(['O'] * len(subwords))
            # list containing [1,2,3, ... , len(words)] per sentence
            # indices are missing if a word is tokenized into subwords
            evaluate_label_ids.append(tid)
            # count number of words
            wid += 1
            # move the token pointer, count number of tokens
            tid += len(subwords)
        assert tid == len(tokens_a)
        evaluate_label_ids = np.array(evaluate_label_ids, dtype=np.int32)
        if id != None:
            examples_tokenized.append((tokens_a, labels_a, evaluate_label_ids, id))
        else:  
            examples_tokenized.append((tokens_a, labels_a, evaluate_label_ids))
        if len(tokens_a) > max_seq_length:
            max_seq_length = len(tokens_a)
      
    # count in [CLS] and [SEP] tokens
    max_seq_length += 2
    #max_seq_length = 128
    if len(examples_tokenized[0]) == 3:
        for ex_index, (tokens_a, labels_a, evaluate_label_ids) in enumerate(examples_tokenized):
            ...
            #    print("labels: %s " % ' '.join([str(x) for x in label_ids]))
            #    print("evaluate label ids: %s" % evaluate_label_ids)
        ...
        for ex_index, (tokens_a, labels_a, evaluate_label_ids, sentence_ids) in enumerate(examples_tokenized):
            #tokens_a = tokenizer.tokenize(example.text_a)
...
def match_ts(gold_ts_sequence, pred_ts_sequence):
    ...
    # collect the number of pos, neg, neutral sentiments in the sentence
    for t in gold_ts_sequence:
        ts_tag = t[2]
        tid = tag2tagid[ts_tag]
        gold_count[tid] += 1

    # for each predicted label, the hit counter is increased if the (beg,end,pol)-triple also appears in the true labels
    # additionally, the counter for the prediction of the corresponding sentiment is increased 
...
def compute_metrics_absa(preds, labels, all_evaluate_label_ids, tagging_schema):
    ...
    # number of true postive, gold standard, predicted targeted sentiment per sentiment
    n_tp_ts, n_gold_ts, n_pred_ts = np.zeros(3), np.zeros(3), np.zeros(3)
    # precision, recall and f1 for aspect-based sentiment analysis per sentiment
    ts_precision, ts_recall, ts_f1 = np.zeros(3), np.zeros(3), np.zeros(3)

    # total number sentences in this batch
    n_samples = len(all_evaluate_label_ids)
    class_count = np.zeros(3)

    # for each sentence in the batch
    for i in range(n_samples):
        # get the word indices per sentence
        evaluate_label_ids = all_evaluate_label_ids[i]

        # get the numeric labels for untokenizable words or first tokens
        # as specified in evaluate_label_ids
        pred_labels = preds[i][evaluate_label_ids]
        gold_labels = labels[i][evaluate_label_ids]
        assert len(pred_labels) == len(gold_labels)

        # turn numeric labels into text tags
        ...
        # turn BIOES label into (no1, no2, polarity)-triple
        # with no1 = start position of predicted aspect
        # and no2 = end position of predicted aspect
        g_ts_sequence, p_ts_sequence = tag2ts(ts_tag_sequence=gold_tags), tag2ts(ts_tag_sequence=pred_tags)

        # get the number of correct predictions, true labels and predictions per sentence
    ...
    # when all sentences in the batch are processed, 
    # calculate precision, recall, f1 micro for each sentiment
    for i in range(3):
        # number of TP
        n_ts = n_tp_ts[i]
        # number of gold labels = TP+FN
        # since they are all truly positive, both detected (TP) and not (FN)
        n_g_ts = n_gold_ts[i]
        # number of predicted labels = TP+FP 
        # since all predictions are counted "positives", both false and true ones
        n_p_ts = n_pred_ts[i]
    ...
    # calculate overall micro-f1 scores
    # TP for all labels
    n_tp_total = sum(n_tp_ts)
    # TP + FN for all labels
    n_g_total = sum(n_gold_ts)
    # TP + FP for all labels
...
def compute_metrics_absa_arts(preds, labels, all_evaluate_label_ids, tagging_schema, ids):
    ...
    for i in range(n_samples):
        ...
        if sentence_id in id_results.keys():
            id_results[sentence_id]["correct"] += sum(hit_ts_count)
            id_results[sentence_id]["total"] += sum(gold_ts_count)
        else:
    ...
    scores = {'macro-f1': macro_f1, 'precision': micro_p, "recall": micro_r, "micro-f1": micro_f1, "acc_ars": acc_ars}
    return scores
...
processors = {
    ...
    "mams": ABSAProcessor,
    "arts-lap": ABSAProcessor,
    "arts-rest": ABSAProcessor,
}
...
output_modes = {
    ...
    "mams": "classification",
    "arts-lap": "classification",
    "arts-rest": "classification",
}